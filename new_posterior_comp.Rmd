...
We can see that the majority of the weight is put on the covariance matrix of t statistics, $X_{t}'X$ and the estimated covariance matrix, $V_{t,1..K}\lambda V_{t,1..K}'$ where here I use K = 13. Recall each V is a $43$ x $1$ vector. 
...
barplot(post.means[j,],main=paste0("PostTissueMeans,B_",j),col=c(1:43))
...
```{r, eval=FALSE}
sfa -gen ./centered.t.stats.txt -t -g 43 -n 6904 -o tri -k 10
```
Then, I will use $K=10$ factor representations of the covariance matrix of $X^{t}X$.
```{r using Factors}
library("mvtnorm")
lambda=as.matrix(read.table("tri_lambda.out"))
factor=as.matrix(read.table("tri_F.out"))

get.prior.covar.Ukl=function(P,L,R,F=10){
  test=list()
  for(l in 1:L){
  test[[l]]=list()
    omega=omega.table[l,]
  ...
  test[[l]][[F+4]]=1/M*(lambda%*%factor)%*%t((lambda%*%factor))}
  
return(U.0kl=test)}

U.0kl=get.prior.covar.Ukl(P=13,L=3,R=43,F=10)
K=length(U.0kl[[1]])
...
#write.table((cbind(as.vector(names.vec),pis$pihat)),quote=FALSE,file="piklhat.txt")
#pi.hat=read.table(file="piklhat.txt")
barplot(pis2$pihat,main="mixEM estimated pi",names=names.vec,las=2)


x=updated.weights(b.gp.hat,se.gp.hat,global.lik)
barplot(as.vector(x),main="naiiveWeights estimated pi",names=names.vec,las=2)
```
