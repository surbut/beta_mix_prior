...
b.gp.hat=na.omit(read.table("50186simulatedbeta.hat.std.txt",header=F,skip=1)[,-c(1,2)])

#se.gp.hat=na.omit(read.table("16008genesnppairs_43tissues_sigma.hat.std.txt",header=F,skip=1)[,-c(1,2)])
se.gp.hat=na.omit(read.table("50186simulatedse.hat.std.txt",header=F,skip=1)[,-c(1,2)])

#t.stat=na.omit(read.table("16008genesnppairs_43tissues_t.stat.txt",header=F,skip=1)[,-c(1,2)])
t.stat=na.omit(read.table("50186simulatedt.hat.std.txt",header=F,skip=1)[,-c(1,2)])
...
#write.table(global.lik,"global.likNOSFA.txt")
```
...
library("SQUAREM")
global.lik=as.matrix(read.table("global.likNOSFA.txt"))
...
```{r postmeanandcovars, cache=TRUE,eval=FALSE}
...
```{r postweightmatrix,cache=TRUE,eval=FALSE}
library("mvtnorm")
...
write.table(post.weight.matrix,"postweightmatrixnoSFA.txt")
```
...
truth=read.table("50186truth.txt")
...
write.table(post.means,"post.means.no.sfa.txt")
write.table(post.ups,"post.ups.no.sfa.txt")
write.table(post.lows,"post.lows.no.sfa.txt")
write.table(post.null,"post.null.no.sfa.txt")
write.table(lfsr.mat,"lfsr.no.sfa.txt")
```

```{r}
post.means=as.matrix(read.table("post.means.no.sfa.txt"))
post.ups=as.matrix(read.table("post.ups.no.sfa.txt"))
post.lows=as.matrix(read.table("post.lows.no.sfa.txt"))
post.null=as.matrix(read.table("post.null.no.sfa.txt"))
lfsr.mat=as.matrix(read.table("lfsr.no.sfa.txt"))
...
snps=c(false[1:9],18530,real[1:8],5166,548)
...
b=barplot(abs(post.means[j,]),main=paste0("PostTissueMeans,B_",truth[j,1]),col=c(1:R),las=2,names=names,beside=TRUE,ylim=c(0,max(abs(post.means[j,]))+0.05))
lfsr=lfsr.mat[j,]
...
```{r}
#global.lik=matrix(unlist(likelihood(b.gp.hat,se.gp.hat)),ncol=K*L,byrow=TRUE)
#write.table(global.lik,"global.like.withsfa.txt")
gl=as.matrix(read.table("global.like.withsfa.txt"))
pis2=mixEM(matrix_lik=gl,prior=rep(1,K*L))
names.vec=matrix(NA,nrow=K,ncol=L)
  for(l in 1:L){ 
    for(k in 1:K){
      names.vec[k,l]=paste0("k=",k,";l=",l)}}

write.table((cbind(as.vector(names.vec),pis2$pihat)),quote=FALSE,file="pi2klhat.txt")
pi.hat=read.table(file="pi2klhat.txt")[,2]
par(mfrow=c(2,3))
  for(l in 1:L){ 
      i=l-1
      start=(i*K+1)
      end=(l*K)
barplot(pi.hat[start:end],main="mixEM estimated pi",names=names.vec[start:end],las=2,col=c(1:14))
}
...
x=updated.weights(b.gp.hat,se.gp.hat,gl)[start:end]
barplot(as.vector(x),main="naiiveWeights estimated pi",names=names.vec[start:end],las=2,col=c(1:14))
}
...
#write.table(post.weight.matrix,"post.weight.matrix.txt")
```

```{r}
post.weight.matrix=as.matrix(read.table("post.weight.matrix.txt"))
```

Here is the code again to compute the component and tissue-specific means.
...
write.table(post.means,"post.means.with.sfa.txt")
write.table(post.ups,"post.ups.with.sfa.txt")
write.table(post.lows,"post.lows.with.sfa.txt")
write.table(post.null,"post.null.with.sfa.txt")
write.table(lfsr.mat,"lfsr.withsfa.txt")

```

And finally, we can plot the posterior means and the lsfr for each tissue.
```{r}
post.means=as.matrix(read.table("post.means.with.sfa.txt"))
post.ups=as.matrix(read.table("post.ups.with.sfa.txt"))
post.lows=as.matrix(read.table("post.lows.with.sfa.txt"))
post.null=as.matrix(read.table("post.null.with.sfa.txt"))
lfsr.mat=as.matrix(read.table("lfsr.withsfa.txt"))



lfsr1=read.table("lfsr.nosfa.txt")
