          ...
          for(k in 1:K){
            ...
            for (l in 1:L){
                  ...
                  all.low[[j]][[k]][[l]]=0
                   ...
                   all.up[[j]][[k]][[l]]=pnorm(rep(0,R),mean=mu.gp1kl,sd=sqrt(diag(U.gp1kl)),lower.tail=F)
                  all.low[[j]][[k]][[l]]=pnorm(rep(0,R),mean=mu.gp1kl,sd=sqrt(diag(U.gp1kl)),lower.tail=T)
                   all.null[[j]][[k]][[l]]=0}
...
temp.up=rbind(as.vector(post.weight.mat[k,l]*upper.list[[k]][[l]]),temp.up)
temp.low=rbind(as.vector(post.weight.mat[k,l]*lower.list[[k]][[l]]),temp.low)
temp.null=rbind(as.vector(post.weight.mat[k,l]*null.list[[k]][[l]]),temp.null)
...
lfsr=apply(rbind(post.ups[x,],post.lows[x,]),2,function(x){1-max(x)})
#pdf(paste0("PostTissueMeans,B_",j))
par(mfrow=c(1,3))
#barplot(post.means[j,],main=paste0("PostTissueMeans,B_",j),col=c(1:43),las=2,names=names[-44])
b=barplot(abs(post.means[x,]),main=paste0("PostTissueMeans,B_",j),col=c(1:R),las=2,names=names,beside=TRUE,ylim=c(0,max(abs(post.means[x,]))+0.05))
text(b, abs(post.means[x,]), labels=round(lfsr,2), pos=3, offset=.5)
legend("topright",legend=round(lfsr,2),col=c(1:R),pch=1,title="lfsr")
#text(barplot(post.means[x,]),1,labels=round(lfsr,2))
#text(post.means[j,],labels=names, srt=45, pos=1,col=c(1:43))
b.mle=t(b.gp.hat[j,])
a.m=abs(b.mle)
a.p=abs(post.means[x,])
low=min(a.m-0.01,a.p-0.01)
high=max(a.m+0.01,a.p+0.01)
plot(abs(b.mle),abs(post.means[x,]),main="PostMeanvsMLE",xlim=c(low,high),ylim=c(low,high),col=c(1:R))

true.m=as.numeric(truth[j,c(4:8)])
plot(abs(true.m),abs(post.means[x,]),col=c(1:R),main="PostMeanvsTrueBeta")
...
# return(U.0kl=test)}
# 
# U.0kl=get.prior.covar.Ukl(P=25,L=2,R=43,F=18)
# K=length(U.0kl[[1]])
# L=length(U.0kl)
