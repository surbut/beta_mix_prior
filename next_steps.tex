...
Similarly, can let $U_{k}$ = $\bm{\gamma}_{k} \bm{\gamma_{k}}^{t}$ where $\bm{\gamma_{k}}$ represents the $1$ by $P$ vector corresponding to the 'kWh' factor from the Sparse Factor representations of the covariance matrix of t statistics, which summarize behavior of all $t_{j}$ in tissue {1..k}$ respectively. 
